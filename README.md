...
You supply the usage data (it isn't auto-detected). CarbonLens maps service+region -> energy (Cloud Carbon Footprint coefficients) -> emissions via live grid intensity. Three inputs: a `mock` demo payload, manual CSV upload, or live cloud-billing adapters (AWS Cost Explorer, GCP BigQuery export, Azure Cost Management) via `uv sync --extra cloud`.

> Scope: a structured *first draft*. It isn't an assured report. Market-based accounting, supplier-specific Scope 3 factors, utilization-aware modeling, and signed PDFs aren't implemented (see [roadmap](#whats-next)). The billing adapters are coded-to-spec and mock-tested, awaiting verification against live accounts.
...
| 11 | Open-Meteo | Worldwide (40+ zones) | **Estimate from weather** | Free, no key |
...
- **Live grid-operator APIs (8):** UK, EIA, OpenElectricity (AEMO), IESO/AESO, Taipower, GridStatus, ENTSO-E, Electricity Maps. These fetch and parse real upstream responses. GridStatus/Electricity Maps need paid keys, ENTSO-E a free token.
- **Heuristic estimators (4):** Grid India, ONS Brazil, Eskom and Québec (time-of-day models) and Open-Meteo (rough intensity inferred from weather). Demo coverage, tagged as such.
...
The scheduled builder also publishes a rolling history archive you can download and analyse freely, with no key and no scraping:
...
| `/api/v1/carbon/signal/{provider}/{region}` | GET | One-call run-now/wait decision with marginal read, a caveat, and clean-surplus flags |
...
carbonlens impact --days 30              # tally of what your runs avoided
...
Gate or annotate a workflow by the live grid, running flexible CI/CD when it's cleanest:
...
Surfaces the marginal/surplus read alongside the average, and documents its limits. See [`.github/actions/carbon-signal`](.github/actions/carbon-signal/README.md). Best for cron-triggered / non-urgent workflows and self-hosted runners, since hosted runners' region is fixed.
...
- **Argo Workflows**: a reusable `carbon-gate` step that branches flexible pipeline work on the grid (`when: clean`). Argo is the dominant k8s-native batch/ML engine, and this makes it carbon-aware with no app changes. See [`deploy/argo`](deploy/argo/README.md).
...
Region choice is the carbon decision with the largest effect (a region can be many times cleaner, permanently). A tiny module sets a provider's `region` from `/carbon/siting` at `apply` time:
...
See [`deploy/terraform/greenest-region`](deploy/terraform/greenest-region/README.md). Ranks by *typical* (history-mean) intensity, the right basis for an always-on deployment.

### Status badge

Embed a live, color-coded carbon-intensity badge (green to red) in any README:
...
- Cloud-billing ingestion adapters (AWS/GCP/Azure): coded-to-spec + mock-tested, awaiting live-account verification

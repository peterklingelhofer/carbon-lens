...
**Carbon Mesh surfaces the underlying grid numbers.** Where a real grid-operator API exists for a region, it serves live gCO2/kWh from that source. Where one doesn't, it falls back to a labeled heuristic or mock value, so the data's provenance is always visible.

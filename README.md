...
Carbon Mesh is a carbon-aware compute platform that guarantees your workloads run on clean energy: actual renewable electrons flowing through the grid at the moment your code executes.
...
Every major cloud provider claims to be "100% powered by renewable energy." Here's what they actually do:
...
| **Google Cloud** | "24/7 Carbon-Free Energy by 2030" | The most transparent of the three. Currently ~64% CFE globally. Some regions (Finland, Iowa) exceed 90%. Others (Singapore, Virginia) are below 30%. Google publishes hourly data, so they know the gap is real. |
...
We call this **real-time carbon arbitrage**, chasing the sun and wind across the globe to keep your workloads on renewable infrastructure.
...
Carbon Mesh works out of the box with 6 no-key providers. Add API keys for 4 more to get real-time government grid data:
...
Each prover network ships **Docker containers** with compiled Rust/C++ provers inside. The broker orchestrates and doesn't compute. The prover runtime module maps all 8 networks to their Docker images and handles witness preparation, container execution with `--gpus all` passthrough, and proof collection.

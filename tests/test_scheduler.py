...
async def test_long_job_is_scored_over_its_whole_window():
    """A multi-hour job's slot intensity is the average over its whole run window,
    including the hours after the start."""
...
async def test_find_optimal_window_lowest_carbon():
    ...
    # Finland (FI) has the lowest carbon, so it should be recommended
...
def test_score_slot_surplus_gets_bounded_edge():
    ...
    # ...but does NOT beat a much cleaner non-surplus slot at 30.

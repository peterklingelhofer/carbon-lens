...
- Clean surplus is intermittent and not guaranteed daily in every zone, so pair with a
...
- Surplus is a heuristic (renewable-share + low carbon + clean margin). Metered
  curtailment is never observed. It's a strong, conservative signal, but zero marginal isn't guaranteed.

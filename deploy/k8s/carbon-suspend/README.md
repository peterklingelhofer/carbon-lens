...
- Shifting helps when the grid is variable and the work is flexible. The
  controller honours the signal's labeling: if the margin is fossil either way it
...
- Decision interval is the controller's own schedule (default every 20 min), so
  suspend/resume is coarse-grained: appropriate for hourly and daily jobs, too slow for minutely ones.

...
These manifests show how Carbon Mesh *would* run on Kubernetes. They're
deployable demos, but hardening for production is still missing. Read this before relying on them.
...
- **The CRD is decorative.** `crd/carbon-route-policy.yaml` defines a
  `CarbonRoutePolicy` resource with a `status` subresource, but **there's no
...
rollout without being one itself.

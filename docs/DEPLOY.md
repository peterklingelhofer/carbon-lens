...
carbon-free grid on a free tier (Iceland geothermal, Nordic and
Québec hydro are paid-region only, see the upgrade path below), but this stack
gets low-carbon for $0 and **uses almost no
...
- **Frontend is a static CDN**: viewer traffic hits cached edge files, never the
...
| **Frontend** | **Cloudflare Workers** (static assets) | Free, global CDN, and the most explicit renewable-matching commitment of the static hosts. (Cloudflare Pages works too. You just can't pin a region on either, since edge nodes are everywhere.) |
...
**Region** to **Oregon** in the dashboard before deploying. Everything else (free
...
This needs no code changes, only a region/host swap:
...
True 24/7 carbon-free (clean in every single hour) is an industry frontier,
...
are present. This makes a free, scale-to-zero public demo possible.
...
**Region** set to **Oregon** (the PNW hydro grid). You get a `https://<name>.onrender.com`
URL.
...
(The snapshot-backed pages, Dashboard and Globe, read GitHub's CDN directly and
...
To serve **real** data from the providers while surviving a traffic
...
database, and everything else is automatic:
...
`CARBON_LENS_ADMIN_SECRET` (matching the API's admin secret). Leave them unset and the job
...
3. Set **Region** to **Oregon**, the one green choice that matters: PNW hydro grid.
...
### Render Blueprint (full stack with Postgres, outside the green/free path)
1. Fork this repo
2. Go to [render.com/deploy](https://render.com) -> New Blueprint Instance
3. Point to your repo, and Render reads `render.yaml` automatically
4. Add API keys in the Render dashboard (Environment tab)
5. Render provisions Postgres + API + Frontend automatically
   (note: the free Postgres expires after 90 days, and this runs always-on, so

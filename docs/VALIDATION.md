...
registered credentials this project does not hold. They are reported as unrun rather than
estimated, and the scripts are complete so anyone with a key can run them. Publishing a
number we did not measure would defeat the purpose of the document.
...
| 6 | How much of the UK/EU difference is accounting basis, not grid? | **MEASURED** | 60.4% at the sampled instant |
...
2. **There is no confidence signal to gate the feature on.** If the backtest shows poor
   agreement, the remedy is to add one, not to threshold an existing one.
...
- They use **solar 45**, which is not an IPCC AR5 row at all. AR5 splits PV into rooftop
...
- They assign **battery discharge world-average intensity (301)**. This project excludes
  ...
  answers to a real question, not one being wrong.
...
Does accounting for imports change the number enough to justify the flow tracer?
...
a decision on a figure that is wrong by 4x for their purpose. This alone justifies the
...
could not be measured. The script classifies each zone rather than leaving that to the reader:
...
- **FR, PT, DK-DK1: correct zeros.** These were net exporters on every evaluable border at
  ...
  to production intensity; that is the tracing equation behaving properly, not a failure.
- **IE: a gap, not a result.** Ireland's only borders in `TRACED_ZONES` are with GB, and GB
  never resolved (below). Ireland had **no evaluable border at all**, so the model gave it
  zero imports by default. Its 0.0% means "not measured", not "no effect".
...
ENTSO-E coverage is not what it was. The flow tracer silently models a smaller network
...
the grid and should not be trusted. This affects the ordinary `/carbon` path for Irish
zones, not just this analysis.
...
next genuinely different value. That difference is the error the carry had accumulated by
the time fresh data arrived: an upper bound on the run, not its mean, and the quantity a
...
**38.2% of all archived points are carried forward, not freshly measured.** That is the
single most important number here and it was not previously stated anywhere.
...
- The **2-4 hour band is the worst**, median 55 gCO2/kWh. That is a real cost, and it is
...
- The 8+ hour band having the *lowest* median is an artefact, not good news. Long holds
  concentrate in stable, low-variance zones; they are the ones where nothing was changing
  anyway.

**Caveat, stated because it bounds the whole result:** a repeated value is not proof of a
carry-forward. A genuinely stable grid can report the same rounded number twice. 423 of the
2,152 runs are single repeats and are the most likely to be coincidental. The measurement
excludes series that never vary at all, but it cannot separate a true carry from a true
...
An intensity of exactly **0.0 gCO2/kWh** is not a grid reading. It is what
...
see**, so a zone whose feed has gone hollow does not merely lose data, it wins every
...
**Cause.** The ENTSO-E adapter already guarded `if not fuel_mix: raise`. But a mix that is
...
grids rather than asserting it is small.

**Mean absolute change 3.9%. Mean signed change +2.0%. Maximum 12.3%. 20 zones up, 9 down.**
...
UK zones do not use this project's emission factors at all. They carry NESO's published
...
is not an artefact of the import factor.

**Consequence.** A UK zone is reported systematically cleaner than an equivalently dirty
fuel-mix zone, and the gap widens as UK renewable output rises, because that is exactly
...
**What was done about it.** Not silently corrected, because both numbers are correct on
their own terms and NESO's is the authoritative figure for the UK. Instead every reading
...
two zones can see that they are not the same quantity. A contract test asserts the two
...
nothing but a network connection, and it will cover a longer window than ours as the
archive grows.

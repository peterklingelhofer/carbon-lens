...
publishes it to a CDN; the app reads that. Quota cost is `O(zones × cadence)` regardless
...
fails if either falls out of sync with the routes. The contract stays in sync by
construction.
...
traffic-light endpoint (`green/yellow/red` + the next cleaner window). They share no

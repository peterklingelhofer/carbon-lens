...
11 is the **onshore** median, and 12 is offshore. The companion project used 12. Neither table
...
| Key | Value | Status |
|---|---|---|
| `lignite` | 1050 | No AR5 row. Above the Coal (PC) published maximum of 910. Two unconfirmed leads recorded in the corpus (a German UBA figure of 1054, and the midpoint of a published lignite range), but neither was read here. |
...
```python
"battery": 0,  # storage: discharge emissions belong to the charging source, so zero is wrong
```
...
**Verdict: PARTIALLY RESOLVED. The methods are implemented correctly, but a reporting
...
and wind data are real, but **nothing published anywhere licenses the step from those to a grid
...
|---|---|---|
| `eskom_heuristic` | ~780 gCO2/kWh base, ×0.92 at midday, ×1.02 at night | No source for the base or the curve shape. South Africa's grid being coal-dominated is well established, but **this specific curve has no such backing**. |
...
hourly matching auditable, but it doesn't itself claim annual matching is weaker. Citing it for

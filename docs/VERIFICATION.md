...
**UNRESOLVABLE**: searched properly, not found, and the product now says so rather than
...
| 2 | The `650` oil factor's provenance | RESOLVED: a 2006 UK POSTnote, not IPCC; tiered C |
...
| 8 | Mixed accounting basis between UK and fuel-mix zones | RESOLVED, surfaced not silently corrected |
...
intent and the value did not match it, so the value was corrected rather than the comment.

Utility is the correct row because balancing-authority fuel-mix feeds report utility-scale
generation; behind-the-meter rooftop does not appear in them at all. EIA's 2023 data,
...
- `coal: 900  # conservative; IPCC median is 820, but subcritical/lignite runs higher`.
  The AR5 median is already a global median across a fleet that is largely subcritical, and
  the argument runs the other way for supercritical units. A 10% upward fudge applied to
  every coal-heavy zone is not conservatism, it is an unstated thumb on the scale. Now 820,
...
said which row it meant, so this was not a disagreement about a value but an undocumented
...
**Verdict: RESOLVED. It is not an IPCC number, and both this project and its companion
implied that it was.**
...
was right that there is no row, and wrong to describe the value as an estimate: 650 was
...
Note what that actually is, because it is much weaker than the use it is put to. The same
page states oil supplied **1% of UK generation** and ran mainly as peaking back-up. So the
figure is a **UK-fleet average, from 2006, for a marginal fuel** — not a global lifecycle
median, not an IPCC value, and now twenty years old. It is tiered **C** accordingly, the
lowest tier of any cited factor.

**Also resolved: where Electricity Maps' `45` for solar comes from.** It appears nowhere in
AR5 despite being labelled "IPCC 2014". It is the midpoint of the rooftop (41) and utility
...
| Key | Value | Status |
|---|---|---|
| `lignite` | 1050 | No AR5 row. Above the Coal — PC published maximum of 910. Two unconfirmed leads recorded in the corpus (a German UBA figure of 1054, and the midpoint of a published lignite range) — neither read here. |
| `waste` | 580 | No AR5 row. Table A.III.11 in the same annex covers waste *treatment*, not waste-to-energy electricity, and does not support this. |
...
It is not conservative in the direction claimed: if the unknown bucket is in fact thermal,
...
left at 300 because there is no evidence to prefer either end and moving it would be an
...
it would not license the number on this corpus's basis even if true.
...
stayed in the **denominator** of the weighted average. The effect is not subtle: a mix of
...
The right treatment is neither 0 nor a positive factor: **storage is not generation**.
Discharge re-delivers energy whose emissions were already attributed when it was generated,
so counting it again double-counts the energy. It is now excluded from both the numerator
...
- The companion project already got this right (`EIA_STORAGE_FUELS = {"BAT", "PS"}`) and
  this project did not. The shared corpus now enforces one answer for both.
...
No source, and it is inferring one physical quantity from another with a straight line and a
...
| Estimator output range | 60.7% – 87.8% |
| True renewable range | 10.3% – 51.2% |

**The two ranges do not overlap.** There is no half-hour in the sample where the estimator
...
output so it cannot be quietly reused. Raw data:
...
- The **code path was genuinely broken**. The `KeyError` was real, and any caller reaching
...
- What was actually wrong for users was the **README's coverage claim**. "UK (18 zones)"
  described a provider capability that did not work and that the API did not expose.
...
the `source` field honestly reporting `open_meteo` or `mock`. The provenance labelling was
working correctly throughout. Only the README's claim was not.
...
CarbonLens does not report every zone on the same basis, and had never said so.
...
**Not corrected, because both numbers are right on their own terms** and NESO's is the
authoritative figure for the UK. Instead every reading now carries
...
Four of those eight strings are not strings any provider emits. Providers stamp
...
Fixed by deriving the grade from the provenance registry, so there is one source of truth
...
All three are **tier D**: self-reported by the vendor, not independently audited. The
...
requirement is not met, and that is now documented.**

The Scope 2 Guidance PDF was downloaded and read (`ghgprotocol.org` HTML pages return 403 to
automated clients, but the PDF does not). The glossary definitions match what
...
caller can produce a one-method report that does not conform. Documented in the calculator's
...
judgement. The standard does not enumerate cloud services.
...
claim, not the weather data. Every `open_meteo` reading now carries
...
|---|---|---|
| `eskom_heuristic` | ~780 gCO2/kWh base, ×0.92 at midday, ×1.02 at night | No source for the base or the curve shape. South Africa's grid being coal-dominated is well established; **this specific curve is not**. |
...
Québec is the weakest: a fixed number that never varies is not an estimate of a time-varying
quantity, it is a constant wearing an estimate's clothes.

## 14. The REC-matching argument

**Verdict: RESOLVED. The argument is sound and is now cited. One source does not say what it
would be convenient for it to say.**

The README's opening argument — that annual REC matching is a weaker claim than hourly
matching — was asserted with no source. It is a real position in the literature and is now
...
hourly matching auditable; it does not itself claim annual matching is weaker. Citing it for
...
Recorded so the document is an audit, not only a defect list.

- The IPCC AR5 Annex III citation itself: correct, and the table is what the code says it is.
...
- The `source` field: already honest everywhere it was checked. When the UK regional zones
  ...
  `open_meteo` or `mock`. The labelling was right; the README's claim above it was not.
...
- The marginal merit-order comment's reasoning (gas rather than coal usually sets the margin,
  because it is the flexible peaker and the ordering is by cost not carbon): correct as

...
function UsualBadge({
  ...
  return (
    <div style={{ fontSize: "0.68rem", marginTop: 2, color: cleaner ? "#4ade80" : "#fbbf24" }}>
      ~{Math.abs(cmp.deltaPct)}% {cleaner ? "cleaner" : "dirtier"} than usual
      {forHour}.
    </div>
  );
}
...
export function RegionWeather({ provider, region }: { provider: string; region: string }) {
  ...
  const {
    ...
    staleTime: 10 * 60_000,
    retry: 1,
    enabled: !snap && !snapshotEnabled, // snapshot has the weather, so never wake the API in prod
  });
  ...
  // Wind turbines cut in around 12 km/h, and solar is meaningful above ~250 W/m2.
    ...
    <div style={{ marginTop: 10 }}>
      ...
      <div style={{ display: "flex", gap: 14, fontSize: "0.8rem" }}>
        ...
        <span title="Shortwave solar irradiance at the surface (drives solar generation)">
          {solar} <span style={{ color: "#9ca3af", fontSize: "0.7rem" }}>W/m² sun</span>
        </span>
      </div>
...
export function RegionBestTime({ provider, region }: { provider: string; region: string }) {
  ...
  const {
    ...
    staleTime: 30 * 60_000,
    retry: 1,
    enabled: !snap && !snapshotEnabled, // snapshot has the greenest hour, so never wake the API in prod
  });

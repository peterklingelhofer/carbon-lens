    ...
    <div style={section}>
      ...
      >
        <div style={card}>
          ...
          {monitorStatus?.running && (
            ...
            >
              {monitorStatus.checks_completed} checks | {monitorStatus.breaches_detected} breaches |{" "}
              {monitorStatus.slas_monitored} SLAs
            </div>
          )}
          <button
            type="button"
            ...
            style={{
              ...
              cursor:
                ...
                (!monitorStatus?.running && (!slas || slas.length === 0))
                  ? "not-allowed"
                  : "pointer",
              opacity: !monitorStatus?.running && (!slas || slas.length === 0) ? 0.6 : 1,
            }}
          >
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                step={5}
                value={minRenewable}
                onChange={(e) => setMinRenewable(Number(e.target.value))}
                style={{ width: "100%" }}
              />
              <div
                style={{
              ...
              <tbody>
                {slas.map((sla) => (
                  <tr key={sla.id} style={{ borderBottom: "1px solid var(--gray-100)" }}>
                    ...
                    <td style={{ ...td, textAlign: "right" }}>{sla.min_renewable_percentage}%</td>
                    ...
                    >
                      {sla.last_checked ? new Date(sla.last_checked).toLocaleString() : "Never"}
                    </td>
                    <td style={td}>
      ...
      {checkMutation.data && (
        ...
        >
          ...
          >
            <StatCard
              label="Avg Carbon"
              value={`${checkMutation.data.avg_carbon_intensity_gco2_kwh}`}
              unit="gCO₂/kWh"
            />
            <StatCard
              label="Max Carbon"
              value={`${checkMutation.data.max_carbon_intensity_gco2_kwh}`}
              unit="gCO₂/kWh"
            />
            <StatCard
              label="Avg Renewable"
              value={`${checkMutation.data.avg_renewable_percentage}`}
              unit="%"
              positive
            />
            ...
            <StatCard
              label="Compliant"
              value={`${checkMutation.data.regions_compliant}`}
              positive
            />
      ...
      {reportMutation.data && (
        <div style={{ ...card, marginBottom: "2rem" }}>
          ...
          >
            Self-generated summary, with no third-party assurance behind it. Checks are in-memory
            and reset on restart, so a freshly-created SLA may summarise a single check over the
            stated period rather than continuous history.
          </div>
          <div
            style={{
              fontSize: "0.8rem",
          ...
          >
            {new Date(reportMutation.data.period_start).toLocaleDateString()} to{" "}
          ...
          >
            <StatCard
              label="Compliance"
              value={`${reportMutation.data.compliance_percentage}`}
              unit="%"
              positive={reportMutation.data.compliance_percentage >= 80}
            />
            <StatCard label="Total Checks" value={`${reportMutation.data.total_checks}`} />
            <StatCard
              label="Compliant"
              value={`${reportMutation.data.compliant_checks}`}
              positive
            />
            <StatCard label="Breached" value={`${reportMutation.data.breached_checks}`} />
            <StatCard
              label="Avg Carbon"
              value={`${reportMutation.data.avg_carbon_intensity_gco2_kwh}`}
              unit="gCO₂/kWh"
            />
            <StatCard
              label="Avg Renewable"
              value={`${reportMutation.data.avg_renewable_percentage}`}
              unit="%"
              positive
            />
          </div>

...
(``run_now`` or clean surplus). Opt in per CronJob with annotations, with no app or
...
to the cluster via the in-cluster service-account token over httpx, with no kubernetes
client dependency. The decision logic is pure and unit-tested.

Limits: this shifts flexible, idempotent CronJobs (backups,
batch ETL, report generation, housekeeping). A suspended CronJob simply skips
fire times while suspended (it doesn't queue and catch up), so only use it
...
def overdue(hours_since_run: float | None, max_defer_hours: float | None) -> bool:
    """True when a job hasn't run within its max-defer window, so run it despite the grid."""
    return (

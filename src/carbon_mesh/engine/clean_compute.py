...
greenest to host on (lowest typical intensity). Pure function so it's unit-tested, and
...
def build_clean_compute_report(
    ...
    carried through only when it has samples and omitted otherwise, so the public report

...
from carbonlens.cli.client import CONFIG_DIR
...
def org_statement(
    ...
    rather than forecast, so a sustainability team can cite it.
    """
    ...
    return {
        ...
        "accounting": (
            "Location-based. CO2 avoided is summed only for jobs that supplied energy (kWh), since "
            "intensity reductions alone aren't additive. Verified jobs re-measured the grid at "
            "execution time. The rest are forecast estimates. An estimate, lacking an assured "
...
def disclosure_markdown(stmt: dict) -> str:
    ...
    per-region breakdown, and the forecast accuracy, so a reader can
    judge both how much was avoided and how trustworthy the estimate is.
    """

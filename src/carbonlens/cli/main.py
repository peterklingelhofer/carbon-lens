...
from carbonlens.cli import client, energy, ledger
from carbonlens.cli.green_run import choose_run_index, choose_run_plan
from carbonlens.cli.ledger import verdict
...
def run(
    ...
    max_intensity: float | None = typer.Option(
        None,
        "--max-intensity",
        help="Run as soon as forecast intensity is at/under this (gCO2/kWh), or "
...
def verify(
    directory: str | None = typer.Option(
        None,
        "--dir",
        help="Directory of per-host *.jsonl ledgers, or omit to use this host's local ledger",
    ),
    org: str = typer.Option("Your organization", "--org", help="Organization name for the header"),
    days: int = typer.Option(90, "--days", help="Reporting period (days)"),
...
def plan(
    ...
    # The plan needs every zone's shiftability, so ask for a
...
def _render_doctor(api: str, checks: dict, ok: bool, json_output: bool) -> None:
    ...
    if "error" in marg:
        ...
        console.print(
            "[red]FAIL[/red] Marginal signal: heuristic, but a marginal key IS configured. No zone "
...
def doctor(
    ...
    try:
        ...
        if unmapped:
            ok = False  # a key was set but no zone mapped, a misconfiguration
    except Exception as e:

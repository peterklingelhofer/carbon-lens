...
class SourceProvenance:
    ...
    `evidence_tier` is resolved, not declared: it is the weakest tier among this
...
_REGISTRY: dict[str, SourceProvenance] = {
    ...
    "uk_carbon_intensity": _p(
        ...
        caveat=(
            ...
            "zones is additionally inferred from the intensity, not reported by NESO."
        ),
    ),
    "electricity_maps": _p(
        "electricity_maps",
    ...
    "hydro_quebec_heuristic": _p(
        ...
        caveat=(
            ...
            "guess, not a reading, and it never changes hour to hour."
        ),
    ),
    "mock": _p(
        "mock",
...
def citation_tiers(citations: tuple[CitationId, ...]) -> tuple[EvidenceTier, ...]:
    """Tiers of the given citekeys. An unknown key counts as E, not as absent."""

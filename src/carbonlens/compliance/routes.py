...
# Process-local stores, so state is lost on restart
...
async def ingest_usage(req: UsageIngestionRequest) -> UsageIngestionResponse:
    ...
    if req.provider == "mock":
        ...
        if req.provider == "aws":
            from carbonlens.compliance.usage_ingestion import AWSCostExplorerAdapter

            adapter = AWSCostExplorerAdapter()
        elif req.provider == "gcp":
            from carbonlens.compliance.usage_ingestion import GCPBillingAdapter

            adapter = GCPBillingAdapter()
        elif req.provider == "azure":
            from carbonlens.compliance.usage_ingestion import AzureCostManagementAdapter

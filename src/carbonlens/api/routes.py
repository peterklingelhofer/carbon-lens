...
from carbonlens.accounting.impact_repo import recent_impacts, record_impact
from carbonlens.accounting.tracker import CarbonTracker, DBCarbonTracker
from carbonlens.api.deps import (
    get_carbon_source,
    get_db_tracker,
    get_engine,
    get_grid_mapper,
...
from carbonlens.engine.recurring import mean_intensity, rank_hours_utc, shiftability_pct
...
from carbonlens.models.accounting import CarbonSavingsReport, ImpactIngest
from carbonlens.models.carbon import (
    BestTime,
    CarbonAnomaly,
    CarbonForecast,
    CarbonHistory,
...
from carbonlens.models.routing import RouteRequest, RouteResponse
...
def _zone_representative(mapper: GridMapper, grid_zone: str) -> CloudRegion | None:
    ...
    history key when looking up a zone directly (on-prem / colo sites)."""
...
async def get_siting(
    ...
    Region choice is a permanent decision with a large effect: a region can be many
...
async def get_methodology(
    ...
    fields = [
        ...
        MethodologyField(
            ...
            notes="Inferred clean oversupply, since curtailment itself is unmetered. Conservative.",
        ),
        MethodologyField(
            field="forecast",
            basis="forecast",
        ...
        MethodologyField(
            ...
            notes="Typical patterns over the window, directional guidance only.",
        ),
        MethodologyField(
...
async def honesty(
    require_measured: bool = Query(
        False, description="Return 503 unless the marginal signal is measured"
    ),
    marginal_source: MarginalSource | None = Depends(get_marginal_source),
) -> dict | JSONResponse:
    ...
    if require_measured and basis != "measured":
        return JSONResponse(
            ...
            content={
                **body,
                "ok": False,
                "reason": "marginal signal is heuristic",
            },
        )
    return body
...
async def ingest_impact(
    ...
    if settings.use_database and session is not None:
        ...
        return {"stored": True}
    return {"stored": False, "detail": "No database configured, so the org ledger is disabled."}

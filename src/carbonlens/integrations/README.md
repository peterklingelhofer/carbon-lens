...
Only gate flexible tasks (nightly batch, retraining, report generation).
...
from carbonlens.integrations.prefect import wait_for_clean_window
...
from carbonlens.integrations.dagster import clean_window_op
...
`pip install dagster`. Both reuse `carbonlens.sdk`, so the decision matches the CLI,
...
```python
from carbonlens.integrations.celery import apply_when_clean

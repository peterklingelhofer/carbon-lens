...
from carbonlens.compliance.routes import router as compliance_router
from carbonlens.config import settings
from carbonlens.logging_config import setup_logging
from carbonlens.scheduler.routes import router as scheduler_router
from carbonlens.sla.routes import router as sla_router
...
async def _warmup_cache() -> None:
    ...
    from carbonlens.api.ws import DEFAULT_REGIONS
...
async def _seed_demo_sla() -> None:
    ...
    from carbonlens.sla.seed import seed_demo_sla
...
def _check_security_posture() -> None:
    ...
    if "*" in settings.cors_origins:
        raise RuntimeError(
            "CORS origins include '*'. Wildcard origins are not allowed, so "
...
app = FastAPI(
    ...
    description=(
        ...
        "- Drafts aimed at **CSRD / SEC Climate / SB 253** workflows, without third-party assurance\n"
...
async def metrics() -> Response:
    ...
    from carbonlens.api.metrics import (
...
async def _check_db() -> str | None:
    ...
    try:
        ...
        from carbonlens.db.engine import AsyncSessionLocal

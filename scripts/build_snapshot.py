...
async def compute_weather(region_meta: dict[str, dict], fetch=_UNSET, concurrency: int = 8) -> dict:
    ...
    weather drivers from the CDN and never calls the live API. Concurrency-limited and best-effort:
...
async def _main() -> int:
    ...
    parser.add_argument(
        ...
        help="Previous snapshot (URL or path) to carry forward from, or '' to disable",
    )
    parser.add_argument(
        "--max-stale-hours",
        type=float,
    ...
    parser.add_argument(
        ...
        help="Rolling history archive output path, or '' to skip",
    )
    parser.add_argument(
        "--history-baseline",
        default="https://raw.githubusercontent.com/peterklingelhofer/carbon-lens/data/history.json",
        help="Previous history archive (URL or path) to append to, or '' to disable",
    )
    parser.add_argument(
        "--no-signals",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Lazy-loaded 7-day per-region forecast (for the clean-window heatmap), or '' to skip",
    )

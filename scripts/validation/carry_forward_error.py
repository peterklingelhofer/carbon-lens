...
carried value, the next genuinely different value, and the elapsed time, and treat
|carried - next| as the error the carry had accumulated by the moment fresh data
arrived. That is an upper bound on the error during the run, not the mean error, and
it is the quantity a consumer of a stale reading actually cares about.

Two honest limits, both handled below:

* A repeated value is not proof of a carry-forward. A genuinely stable grid can
